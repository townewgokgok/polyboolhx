...
class Intersecter {
	...
	public static function intersecter(selfIntersection: Bool, eps: Epsilon, ?buildLog: BuildLog): IIntersecterResult {
		...
		function calculate(primaryPolyInverted: Bool, secondaryPolyInverted: Bool): Array<ISegment> {
			...
			function statusCompare(ev1: INode, ev2: INode): Float {
			...
			while (!event_root.isEmpty()) {
				...
				if (ev.isStart){
					...
					if (st == null){
						throw 'PolyBool: Zero-length segment detected; your epsilon is ' +
		...
		if (!selfIntersection) {
			...
			return {
				calculateNSI: function(segments1: Array<ISegment>, inverted1: Bool, segments2: Array<ISegment>, inverted2: Bool): Array<ISegment> {
		...
		return {
			...
			calculateSI: function(inverted: Bool): Array<ISegment> {
